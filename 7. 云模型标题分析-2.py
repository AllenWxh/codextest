...
async def main_async(input_file, output_file):
    # 明确读取 post_title 字段（适配 potential_hotposts.json）
    ...
    async with aiohttp.ClientSession(timeout=timeout, connector=connector) as session:
        tasks = [analyze_title(session, row["post_title"], i) for i, row in df.iterrows()]
        completed = 0
    ...
    print("\n相关帖子（置信度 ≥ 0.70）：")
    print(relevant[["post_title", "confidence"]])

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--input", type=str, default="potential_hotposts.json")
    parser.add_argument("--output", type=str, default="related_posts_2.json")
